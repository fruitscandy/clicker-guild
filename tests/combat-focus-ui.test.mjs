...
test("combat focus mode keeps only the field and attack feedback visible", async () => {
  ...
  for (const selector of [
    ".battle-mode .field-toolbar",
    ".battle-mode .battle-sidebar",
    ".battle-mode .swarm-hud",
    ".battle-mode .loot-tally",
  ...
  assert.match(focusStyles, /\.battle-mode \.arena \{[\s\S]*?min-height: 100svh;/);
