...
> **작업 디렉터리 격리 필수:** 모든 독립 세션은 다른 세션과 공유하지 않는 고유한 작업 디렉터리를 사용한다. Codex 프로젝트에서는 새 `worktree` 환경을 사용한다. 프로젝트를 사용하지 않고 세션마다 새 디렉터리에 저장소를 복제했다면 그 복제본 자체를 독립 작업공간으로 인정하며 추가 worktree는 필요하지 않다. 기존 또는 공용 체크아웃을 재사용해 수정하려면 반드시 새 worktree를 만든다. 다른 세션의 디렉터리를 재사용하거나 같은 디렉터리에서 브랜치만 전환하지 않는다.
...
7. 최신 `origin/main`에서 작업별 `codex/<짧은-작업명>` 브랜치를 만든다. Codex 프로젝트나 기존 체크아웃을 사용한다면 새 별도 worktree에서 만들고, 세션 전용 새 clone이라면 그 격리된 복제본에서 만든다.

공용 체크아웃은 최신 `main`을 fast-forward로 갱신하고 개발 서버를 실행하는 읽기 전용 확인 공간으로 사용할 수 있다. 공용 체크아웃에서는 파일 수정, 커밋, 작업 브랜치 또는 PR 브랜치 체크아웃을 하지 않는다. PR 실행·검증은 PR별 임시 worktree에서 수행한다.
